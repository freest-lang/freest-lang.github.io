...
Let us start with the conventional unrestricted values. All constants are unrestricted, hence the following program compiles and prints `10`{: .language-freest }. Notice that value `n`{: .language-freest } is used twice.
```freest
copy : ()
copy = 
...
Unrestricted values need not be used at all. The program below still prints `10`{: .language-freest } but never uses the value in variable `n`{: .language-freest }.
```freest
discard : ()
discard = 
  let n = 5 in print 10
...
discard = 
  print 20
```
In either case, expect to read `20`{: .language-freest } on the console.

The type constructor for an unrestricted function is usually written as `->`{: .language-freest }. But that is an abbreviation for `-*->`{: .language-freest }, where the explicit `*`{: .language-freest } points to an unrestricted value. Function `double`{: .language-freest } could have been written as follows.
```freest
double : Int -*-> Int
double x = x + x
```
...
Linear functions seem simple; however, there is more to them than meets the eye. The type `Int -1-> Int`{: .language-freest } describes a linear function that takes an integer and returns an integer, but what about type `Int -1-> Int -*-> Int`{: .language-freest }? This type also describes a linear function, but with a twist: the function is linear on its first argument only. If one partially applies the function, then they are left with an unrestricted function. This is easy to see if one recalls that `Int -1-> Int -*-> Int`{: .language-freest } stands for `Int -1-> (Int -*-> Int)`{: .language-freest }.
```freest
f : Int -1-> Int -*-> Int
f x y = x + y
...
In the above example, after partially applying `f`{: .language-freest } we are left with an unrestricted function `f'`{: .language-freest } (of type `Int -*-> Int`{: .language-freest }) which we then use multiple times.

Let us now analyse a type, similar in form but of quite different behaviour: `Int -*-> Int -1-> Int`{: .language-freest }. This is more tricky than what we've seen until now. Let us analyse it step by step: look at it as `Int -*-> (Int -1-> Int)`{: .language-freest }. Now it's clearer that it represents an **unrestricted** function that takes an integer and returns another **linear** function that takes an integer and returns an integer.
```freest
g : Int -*-> Int -1-> Int
g x y = x + y
...
partialUnApplication = 
  ...
  print (g1 3 + g2 4)
```
This time, function `g`{: .language-freest } is used multiple times to create linear functions `g1`{: .language-freest } and `g2`{: .language-freest }, each of which is then used exactly once. Expect to read integer `10`{: .language-freest } on the console.

Functions `f`{: .language-freest } and `g`{: .language-freest } are fundamental to understand how functions deal with linearity and how we programmers should both write and use them. From here on out, there are many possible cases that combine both cases of `f`{: .language-freest } and `g`{: .language-freest }, but by breaking it down type by type as we did above, we can quickly understand any combination of linear and unrestricted functions.
